...
public static class EditionSplitFoundationTests
{
    public static async Task Run(Action<bool,string> assert)
    {
        ...
        // K-3: setup still ships with documented recovery credentials, but
        // they are only an entry into the mandatory admin credential dialog.
        // The session itself is powerless until a >=10-character password and
        // a non-factory PIN have been stored.
        ...
        try
        {
            ...
            assert(
                ...
                "the shipped admin credentials open only a powerless must-change session");
            ...
            assert(
                shortRejected,
                "admin credential replacement enforces the reviewed 10-character password minimum");
        }
        finally
        {
            Microsoft.Data.Sqlite.SqliteConnection.ClearAllPools();
