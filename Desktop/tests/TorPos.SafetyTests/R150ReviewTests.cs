...
public static class R150ReviewTests
{
    public static async Task Run(Action<bool, string> assert)
    {
        ...
        assert(
            ...
            "R150 Marktwertmethode splits 9.00 EUR menu from 7.00/3.00 market values into 6.30 EUR at 7% and 2.70 EUR at 19%");
        ...
        assert(
            ...
            "R150 Im-Haus rule is applied to menu components before mixed-rate decision");

        var promoted = MenuVatPolicy.Analyze(
            menu,
            catalogProducts,
            imHaus: false,
            menuGrossCents: 850);
        assert(
            ...
            "R150 actual sold menu price is allocated proportionally with deterministic cent rounding");

        var overpriced = MenuVatPolicy.Analyze(
            menu,
            catalogProducts,
            imHaus: false,
            menuGrossCents: 1100);
        assert(
            !overpriced.IsValid &&
            overpriced.Message.Contains("übersteigt"),
            "R150 refuses menu price above known market values instead of inventing a VAT allocation");
        ...
        var depositAnalysis = MenuVatPolicy.Analyze(
            ...
            imHaus: false);
        assert(
            !depositAnalysis.IsValid &&
            depositAnalysis.Message.Contains("Pfand"),
            "R150 combo component Pfand is blocked until it can be represented as its own DSFinV-K position");
        ...
        var inHouseSnapshot = snapshot with
        {
            OperationId = "r150-inhouse",
            ImHaus = true,
    ...
    {
        public FakeCatalog(IReadOnlyList<Product> products) =>
            Products = products;

        public IReadOnlyList<ProductGroup> Groups =>
    ...
    private sealed class FakeJournal : ICheckoutJournal
    {
        ...
        public Task<IReadOnlyList<CheckoutOperation>> GetOpenAsync() =>
            Task.FromResult<IReadOnlyList<CheckoutOperation>>(
                _operation is null
        ...
        public Task<CheckoutOperation?> GetAsync(string id) =>
            Task.FromResult(
                _operation?.Snapshot.OperationId == id
                    ? _operation
                    : null);
    ...
    private sealed class FakeTerminal : IPaymentTerminalService
    {
        ...
        public Task<PaymentTerminalPaymentResult> PayAsync(
            long amountCents,
            string operationId,
            CancellationToken ct = default)
        {
            ...
            return Task.FromResult(
                new PaymentTerminalPaymentResult(
                    true,
                    "APPROVED",
                    "approved",
