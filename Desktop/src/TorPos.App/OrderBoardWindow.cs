...
public sealed class OrderBoardWindow : Window
{
 ...
 public OrderBoardWindow(OrderWorkflowService service,bool training,string actor)
 {
  ...
  actions.Children.Add(Button("ZUM KASSIEREN / BEARBEITEN",()=>{if(list.SelectedItem is OrderOverview row&&row.Open)Close((long?)row.Id);else status.Text=UiLanguage.T("Nur unbezahlte Bestellungen können aufgerufen werden.");return Task.CompletedTask;}));
  ...
  status.Text=UiLanguage.T("Status ändern erzeugt keine Zahlung. Hinweise werden beim nächsten Küchenbon mitgedruckt. Maximal 500 Bestellungen.");
 ...
 async Task Reload(){try{rows=await service.ListAsync(training,all.IsChecked==true);Filter();}catch(Exception ex){status.Text=UiLanguage.T("Laden fehlgeschlagen")+": "+ex.Message;}}
 ...
 async Task Save(string state){if(list.SelectedItem is not OrderOverview row){status.Text=UiLanguage.T("Bitte Bestellung auswählen.");return;}await service.UpdateAsync(row.Id,row.Version,state,note.Text??"",actor,training);await Reload();status.Text=UiLanguage.T("Gespeichert. Zahlungsstatus bleibt unverändert.");}
}
