...
public sealed class RestaurantWaiterSettlementService
{
    ...
    private async Task<RestaurantWaiterSettlement> BuildCoreAsync(DateTimeOffset from, DateTimeOffset to, CancellationToken ct)
    {
        ...
        if (await TableExistsAsync(c, "restaurant_sessions", ct))
        {
            ...
            {
                q.CommandText = """
                    SELECT s.id, s.assigned_waiter, i.quantity_milli, i.unit_price_cents
                    FROM restaurant_sessions s
                    LEFT JOIN restaurant_session_items i ON i.session_id=s.id AND i.state='ACTIVE'
                ...
                while (await r.ReadAsync(ct))
                {
                    ...
                    var cents = r.IsDBNull(2) ? 0L : LineTotal(r.GetInt64(2), r.GetInt64(3));
    ...
    /// <summary>Same rounding as RestaurantSessionItem.LineTotalCents.</summary>
    private static long LineTotal(long quantityMilli, long unitPriceCents) =>
        (long)Math.Round(quantityMilli / 1000m * unitPriceCents, MidpointRounding.AwayFromZero);
