    ...
    private async Task<ExportPlan> BuildPlanAsync(DateTimeOffset from, DateTimeOffset to, CancellationToken ct)
    {
        return await IoQueue.RunAsync(async () =>
        {
            ...
            var open = await ScalarLongAsync(c,
                "SELECT (SELECT COUNT(*) FROM sales WHERE created_at_utc > $from) + (SELECT COUNT(*) FROM cash_movements WHERE created_at_utc > $from AND movement_type IN ('EINLAGE','ENTNAHME') AND fiscal_mode <> 'TEST_ONLY') + (SELECT COUNT(*) FROM training_receipts WHERE created_at_utc > $from) + (SELECT COUNT(*) FROM aborted_vorgaenge WHERE ended_at_utc > $from) + (SELECT COUNT(*) FROM order_bestellungen WHERE created_at_utc > $from) + (SELECT COUNT(*) FROM restaurant_bestellungen WHERE created_at_utc > $from);",
