...
public sealed class TseVorgangService
{
    ...
    /// transaction the TSE finishes that SAME transaction.
    ///
    /// KassenSichV § 2 requires the transaction to start immediately with the
    /// Vorgang. If that start failed because the TSE was unavailable, TOR must
    /// preserve that Vorgang as an outage. It must NOT start a fresh transaction
    /// at payment time and present the later TSE time as if it were the original
    /// start. A direct Start+Finish fallback is only valid where no earlier
    /// Vorgang existed (for example a recalled unchanged order whose Kassenbeleg
    /// legitimately starts with payment).
    /// </summary>
    public async Task<SaleTseResult> FinishAsync(string vorgangId, string processType, string processData, string actor, string reference, CancellationToken ct = default)
    {
        ...
        if (vorgang is { State: Open or Parked } started)
        {
            ...
            {
                // The legally decisive TSE start was missed. Do not "repair"
                // it with a later transaction carrying the wrong start time.
